...
const logoPath = path.join(__dirname, '../images/logo.png'); // still referenced for clarity

// 🔹 Microsoft Graph API client
const getGraphClient = () =>
  Client.init({
    authProvider: (done) => {
      done(null, process.env.MS_GRAPH_TOKEN); // ⚠️ make sure this token is valid
    },
  });

// 🔹 Helper to send email through Microsoft Graph
async function sendEmailGraph(to, subject, htmlContent) {
      ...
      attachments: [
        {
          ...
          isInline: true,
          contentBytes: require('fs').readFileSync(logoPath).toString("base64"),
        },
      ],
    },
  ...
  await client.api("/users/" + process.env.MS_USER_ID + "/sendMail").post(message);
}

// 🔹 Styled HTML generator (same as your version)
...
// 📧 Reset Password Email (refactored)
exports.sendResetPasswordEmail = async (to, resetLink) => {
  ...
  await sendEmailGraph(to, 'Reset your Connect2Uni password', html);
};




//gamil smtp code for sending otp mail
